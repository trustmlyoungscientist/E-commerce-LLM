...
from utils.prompter_llama import Prompter
import pdb
os.environ["TOKENIZERS_PARALLELISM"] = "false"
os.environ['TMPDIR'] = '/data/ptan499/tmp'
import sys
sys.path.append("/data/ptan499/python_packages")
...
def train(
    ...
    if base_model == 'microsoft/phi-2':
        ...
        model = AutoModelForCausalLM.from_pretrained(
            ...
            trust_remote_code=True,use_auth_token='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')

        tokenizer = AutoTokenizer.from_pretrained(base_model, use_auth_token='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')
        bf16 = False
        ...
        model = AutoModelForCausalLM.from_pretrained(
            ...
            torch_dtype=torch.bfloat16,
            device_map=device_map,
            trust_remote_code=True, attn_implementation="flash_attention_2",use_auth_token='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')
    ...
    #     full_prompt = prompter.generate_prompt(
    #         data_point["input_field"],
    #         data_point["output_field"], 
    #         )
    ...
    if dev_data_path.endswith(".json") or dev_data_path.endswith(".jsonl"):
        ...
        dev_data = load_dataset('parquet', data_files=dev_data_path)
    elif data_path.endswith(".csv"):
        data = load_dataset('csv', data_files=data_path)
    else:
